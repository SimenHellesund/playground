...
Need h5py and elasticsearch python packages installed to run.

The difference between this file and downloading_final, is that this scipt does not collect packetloss and latency information from the Chicago instance of kibana.
...
now = 1504915200000#int(time.time() - 60*60) * 1000  #epoch milliseconds                                                   
past = now - interval
...
for entry in metrics:
	if "closeness" in metrics[entry].keys():
		closeness[entry.upper()] = metrics[entry]["closeness"]["latest"]
...
#store variable list as a python list in a python file. That way it can be easily imported using "from varList import variables"
with open("varList.py", 'wb') as fp:
        fp.write("variables = [")
