...
now = 1505573241000#int(time.time() - 60*60) * 1000  #epoch milliseconds                                                   
past = now - interval
...
numpy_total = np.concatenate((numpy_closeness,numpy_throughput_link,numpy_done_link_6h,numpy_done_link_1h,numpy_queued_link,numpy_size,numpy_protocol,numpy_retried,numpy_packetloss,numpy_latency,numpy_successful), axis=1)

#list to keep track of the order of variables. Remember to update this when "numpy_total" changes
variables = ["closeness","throughput","done_6h","done_1h","queued","size","protocol","retried","packetloss","latency"]
